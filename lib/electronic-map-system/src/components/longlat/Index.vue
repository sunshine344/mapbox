...
<template>
    <div id="longlatinfopoup" v-if="flagBoolean"> 
        ...
        <Form  :model="formState"  @finish="onFinish"  @finishFailed="onFinishFailed">
            <FormItem label="经度" name="long" >
                <AInput v-model:value="formState.long"    ></AInput>
            </FormItem>
             <FormItem label="纬度" name="lat" >
...
export default defineComponent({
    ...
    setup (props,context) {
           let formState = reactive({long:'',lat:''} as FormState);
           let flagBoolean:Ref<boolean>=ref(false)
           const longconfig:AnyObject = {
                rules: [{ type: 'string' as const, required: true, message: '请填写经度(数字)' }],
            }
            const latconfig:AnyObject = {
                rules: [{ type: 'string' as const, required: true, message: '请填写纬度(数字)' }],
            }
            ...
            const show=(e:FormState)=>{
                 formState.long=e.long;
                 formState.lat=e.lat;
