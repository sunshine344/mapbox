...
import TEXTBOX from "@/assets/images/map/textbox.png";
// import {PolylineTrailMaterialProperty} from "./lib/PolylineTrailMaterialProperty/PolylineTrailMaterialProperty "
import LINE_IMG from "./lib/PolylineTrailMaterialProperty/light.png";
import heatmapjson from "./heatmap.json"
import  {CesiumHeatmap }from "cesium-heatmap-es6"
import FlyLineLayer from "@assets/js/flyLine/flyLine.js"
console.log('热力图',heatmapjson)
var configs = {
    d3map: null,
    mapDom: "map3d",
...
function supermap() {
    ...
    this.PolylineTrailMaterialPropertyHandler=null;//光线分析事件点击监听方法
    this.preOdPolylineTrailMaterialEnty=null;//保存之前od分析方法
}
/**
 * @Descripttion: 初始化地图
 * @Author: 陈杰
...
supermap.prototype.init = function (opt = {}) {
    if (configs.mapDom && configs.mapUrl) {
        ...
        //     mapId: 'mapbox.dark'
        // }))
        this._viewer.scene.multiViewportMode=1;
    ...
    supermap.prototype.movepostion=function(calback){
        ...
        handler.setInputAction(function (movement) {
            ...
            if(cartesian){  
                ...
                let alt=Cesium.Math.toDegrees(cartographic.height).toFixed(2);
                let height = Math.ceil(_this._viewer.camera.positionCartographic.height);
                let zoom =altitudeToZoom(height)
                calback?calback({position:[long,lat,alt],zoom:zoom}):null
 ...
 * @Descripttion: 移除图层
 * @Author: 陈杰
 * @Date: 2022-02-17 19:57:09
 * @LastEditors: 
 * @param {*} eny
 * @return {*}
 */
supermap.prototype.removelayer=function(eny){

    this._viewer.imageryLayers.remove(eny)
}
/**
 ...
 * @return {*}
 */
supermap.prototype.PolylineTrailMaterialProperty=function(data,calback){    
        let viewer=this._viewer;
        let flylineobj=new FlyLineLayer('flyline',viewer);
        this.PolylineTrailMaterialaddLisener((e)=>{
            calback?calback(e):null
        })
      return   flylineobj.create(data)

}
supermap.prototype.removEventPolylineTrailMaterialProperty=function(){
    if(this.PolylineTrailMaterialPropertyHandler){
        this.PolylineTrailMaterialPropertyHandler.destroy();
        this.PolylineTrailMaterialPropertyHandler=null;
        this.preOdPolylineTrailMaterialEnty=null;
    }


}
supermap.prototype.PolylineTrailMaterialaddLisener=function(calback){
    let viewer=this._viewer;
    let _this=this;
    if(this.PolylineTrailMaterialPropertyHandler){
        this.PolylineTrailMaterialPropertyHandler.destroy();
        this.PolylineTrailMaterialPropertyHandler=null;
        this._this.preOdPolylineTrailMaterialEnty=null;
    }
    this.PolylineTrailMaterialPropertyHandler=new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas);
    this.PolylineTrailMaterialPropertyHandler.setInputAction(function(movement){
        let pick=viewer.scene.pick(movement.position);
        if(pick){
            if(pick.id instanceof Cesium.Entity){
                // let ellipsoid=this._viewer.scene.globe.ellipsoid;
                let Id=pick.id;
                if(Id._name=="PolylineTrailLink"){
                    // Id._polyline._material.color._value=Cesium.Color.fromCssColorString('#f0f')
                    // let values=Id.entityCollection.values;
                  
                    if(_this.preOdPolylineTrailMaterialEnty){
                        _this.preOdPolylineTrailMaterialEnty.polyline.material=new Cesium.PolylineTrailLinkMaterialProperty(
                            // new Cesium.Color.fromCssColorString(data[i].color),
                            new Cesium.Color.fromCssColorString('#0ff'),
                            3000
                        )
                    }
                    Id.polyline.material= new Cesium.PolylineTrailLinkMaterialProperty(
                        // new Cesium.Color.fromCssColorString(data[i].color),
                        new Cesium.Color.fromCssColorString('#FFFF00'),
                        3000
                    )
                    calback?calback(Id.properties):null;
                    _this.preOdPolylineTrailMaterialEnty=Id;
                    // values[0]._polyline._material.color._value=Cesium.Color.fromCssColorString('#f0f')
                   
                    // values.forEach((item,i)=>{
                    //         if(item.name=="PolylineTrailLink"){
                    //             i==1?item._polyline._material.color._value=Cesium.Color.fromCssColorString('#f0f'):null
                    //         //    n<1? item._polyline._material.color._value=Cesium.Color.fromCssColorString('#f0f'):null;
                    //         //     n++;
                    //         }
                    // })
...
// this.PolylineTrailMaterialPropertyHandler
/**
 * @Descripttion: 
 * @Author: 陈杰
...
// 移除地形
supermap.prototype.removeterrian=function(terrian){
 ...
 * @return {*}
 */
 supermap.prototype.addPointCirclekcenter=function(pointArray=[{ point:[104,34,50],color:'rgba(251,0,252,0.8)'} ],option={name:"pointmark",circlr:'',fillColor:"#fff",outlineColor:'"#000"',pixelOffset:[0,0],billboardpixelOffset:[0,0]}){
    var  viewer=this._viewer;
    let _this=this;
                ...
                pointMarkCollection.entities.add(
                        {
                            ...
                            ellipse: {
                                semiMinorAxis: 50,
                                semiMajorAxis:50,
                                material:Cesium.Color.fromCssColorString(item.color?item.color:'rgba(251,0,252,0.8)'),
                                outline: true,
                                outlineColor:Cesium.Color.fromCssColorString(item.color?item.color:'rgba(251,0,252,0.8)'),
                                heightReference:'',
                              },
                            label: {
                                ...
                                fillColor: Cesium.Color.fromCssColorString(option.fillColor?option.fillColor:"#fff"),
                                outlineColor: Cesium.Color.fromCssColorString(option.fillColor?option.fillColor:"#fff"),
                                // backgroundColor : Cesium.Color.fromCssColorString(option.fillColor?option.fillColor:"#fff"),
                                // backgroundPadding : new Cesium.Cartesian2(10, 10),
                                // style : Cesium.LabelStyle.FILL,
                                outlineWidth:3,
                                showBackground: false, //确定是否显示该标签后面的背景。
                                verticalOrigin: Cesium.VerticalOrigin.CENTER,
...
supermap.prototype.thermodynamic=function(points,max){
    let _this=this;
    let viewer=this._viewer;
    var heatmapInstance = h337.create({
        // only container is required, the rest will be defaults
        container:document.getElementById('heatmapbox')
    });
    let pointsArray=[];
    points.forEach(item=>{
            let wwp=new Cesium.WebMercatorProjection();
            wwp.project(Cesium.Cartographic.fromDegrees(item.x,item.y))

    })
    // now generate some random data
    // {x: 104.026465, y: 30.562833}
    //{x: 104.106198, y: 30.564609}
    // {x: 104.104072, y: 30.520508}
    // {x: 104.021124, y: 30.526064}
    // var points = [];
    // var max = 0;
    // var width =600;
    // var height =600;
    // var len = 95;
    var latMin =20.2141799223;
    var latMax =30.564609;
    var lonMin = 109.650789918;
    var lonMax =  117.313711727;
    // while (len--) {
    ...
    let enty= viewer.entities.add({
        ...
        // classificationType:Cesium.ClassificationType.CESTUM_3D_TILE,
...
supermap.prototype.heatmapav=function(points,max){
    let viewer=this._viewer;
    const earthHeatMap = new CesiumHeatmap(viewer,
        {
            zoomToLayer: true,
            points,
            heatmapDataOptions: { max: 200, min: 0 },
            heatmapOptions: {
                maxOpacity:1,
                minOpacity: 0
            }
 ...
 * @Descripttion:导出图片
 * @Author: 陈杰
 * @Date: 2022-02-21 13:21:08
 * @LastEditors: 
 * @param {*}
 * @return {*}
 */
supermap.prototype.printImage=function(){
    let viewer=this._viewer;
