 ...
 * @LastEditors  : Pat
 * @LastEditTime : 2022-04-12 15:06:04
 */
import {
	defineComponent,
	App,
...
const CurrentComponent = defineComponent({
	name: 'Message',
	props: {
	...
	setup(props) {
		const messageState: Ref<boolean> = ref(true),
			...
			time = (callback: (agrs?: number) => void, duration: number) =>
				setTimeout(callback, duration);
		const customStyle: any = ref();
		watchEffect(() => {
			const currentStyle: AnyObject = { zIndex: props.zIndex };
		...
		return { messageState, messageClass, customStyle };
	},
});
/**
 ...
 * @author: Pat
 */
CurrentComponent.render = function (_ctx: any): any {
	const setupAnimationString: string = _ctx.animation,
		setupPosition: string = _ctx.position;
						...
						withDirectives(
							createVNode('div', {
								...
								class: [
									'el-info-message',
									setupPosition,
									setupAnimationString,
									`${setupAnimationString}-animation`,
									`el-msg-${_ctx.type}`,
									_ctx.messageClass,
								],
...
CurrentComponent.install = (app: App<Element>) =>
