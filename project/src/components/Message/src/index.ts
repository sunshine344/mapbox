 ...
 * @LastEditors  : Pat
 * @LastEditTime : 2022-04-12 14:59:12
 */
...
const Message: any = function (opts: AnyObject | string = {}) {
	...
	const id = 'message_' + seed++,
		callback: (...agrs: any[]) => void = options.onClose,
 ...
 * @author: Pat
 */
function close(id = '', callback?: (vm: AnyObject) => void) {
	...
	for (let i = idx; i < len; i++) {
		const props: AnyObject = instances[i].vm.component.props;
		const pos =
			parseInt(instances[i].vm.el.style[props.position], 10) -
			removedHeight -
			16;
		props.offset = pos;
