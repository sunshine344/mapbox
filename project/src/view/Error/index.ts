 ...
 * @LastEditors  : Pat
 * @LastEditTime : 2022-04-12 14:32:36
 */
import './error.scss';
...
function initMath(math: Array<string | null>): any {
	let i = 0;
	const [time, timr]: [number, Array<number | any>] = [20, []];
...
export default defineComponent({
	setup() {
		let timr: Array<number | any> = [];
		const math: Array<string | null> = ['4', '0', '4'],
			msg = '访问的页面不存在！';
		...
		return () => (
			...
			createBlock('div', { class: 'error' }, [
				...
				createBlock(
					'ul',
					null,
					renderList(list, () => (openBlock(), createBlock('li'))),
					128 /* KEYED_FRAGMENT */,
				)),
			])
		);
