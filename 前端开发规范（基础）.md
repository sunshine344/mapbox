...
- 书写过程中, 每行代码结束必须有分号; 原则上所有功能均根据 XXX 项目需求原生开发, 以避免网上`down`下来的代码造成的代码污染(沉冗代码 || 与现有代码冲突 || ...);
...
- 方法名、参数名、成员变量、局部变量都统一使用 `lowerCamelCase` 风格，必须遵从驼峰形式。 #### Counter example `js _name // or name_ // or name$ ` #### Positive example `js localValue // or getHttpMessage() // or inputUserId `
...
- 超过 80 个字符的字符串应该使用字符串连接换行(如果过度使用，长字符串连接可能会对性能有影响.)
  #### Counter example
  ```ts
...
- 函数表达式
  ...
  - 绝对不要在一个非函数块里声明一个函数，把那个函数赋给一个变量。浏览器允许你这么做，但是它们解析不同。 （ECMA-262 定义把块定义为一组语句，函数声明不是一个语句。）
